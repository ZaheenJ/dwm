...
resizeclient(Client *c, int x, int y, int w, int h)
{
	...
	for (n = 0, nbc = nexttiled(c->mon->cl->clients, selmon); nbc; nbc = nexttiled(nbc->next, selmon), n++);
...
bstack(Monitor *m) {
	...
	for (i = mx = 0, tx = m->wx, c = nexttiled(m->cl->clients, selmon); c; c = nexttiled(c->next, selmon), i++) {
		if (i < m->nmaster) {
